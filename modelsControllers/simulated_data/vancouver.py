...
def save_results(vancouver_graph=None, experiment_df=None, experiment_id=None):

    for item in vancouver_graph.items:

        experiment_df.at[item.id, 'Vancouver'] = round(item.grade, 20)

    experiment_file = STUDY_DIR + str(experiment_id) + '.csv'
    return experiment_df.to_csv(experiment_file, index=False)
...
def run(experiment_id):
    ...
    experiment_df = get_df(experiment_id)

    g = Graph()

    for student_id, pd_student in experiment_df.iterrows():

        student = Student(experiment_df, student_id)
        student_grades = list(student.get_grades())
        student_graders = list(student.get_graders())

        for index, grade in enumerate(student_grades):

            # student_id, grader_id, peer_grade
            g.add_review(student_graders[index], student_id, grade)

    g.evaluate_items()

    save_results(g, experiment_df, experiment_id)
